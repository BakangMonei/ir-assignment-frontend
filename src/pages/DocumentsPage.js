    ...
    <div className="space-y-4">
      ...
      >
        ...
        {isLoading ? (
          ...
          <EmptyState title="No documents found" description="Try changing filters or create a new document." />
        ) : (
          <div className="overflow-auto rounded-md border border-gray-200">
            <table className="min-w-full text-sm">
              <thead className="bg-gray-50">
                <tr className="text-left text-gray-500">
                  <th className="px-3 py-2">ID</th><th>Title</th><th>Category</th><th>Year</th><th />
                </tr>
              </thead>
              <tbody>
                {rows.map((doc) => (
      ...
      {selectedDoc && (
        <Card title="Document Details">
          <pre className="overflow-auto rounded bg-gray-100 p-3 text-xs">{JSON.stringify(selectedDoc, null, 2)}</pre>
        </Card>
      )}
    </div>
  );
